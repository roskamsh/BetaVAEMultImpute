## Figure 2 - ranked plots for all imputation strategies, but subsetted for a small number of indices
## Beta = 2, epochs = 250
...
f1a <- ggplot(data = na_vals_is_melt, mapping = aes(x = NA_rank, y = value)) +
  ...
  theme_bw() +
  ylim(c(min(na_vals_is_melt$value) - 2, max(na_vals_is_melt$value + 1))) +
...
pdf("../../ICLR_paper/figures/Single_versus_multiple_imputation_NA_index_1000points_beta2_epochs250.pdf", width = 7, height = 5)
plot_grid(f1d, f1a, labels = c("a","b"))
dev.off()

pdf("../../ICLR_paper/figures/Single_versus_all_imputation_strategies_NA_index_1000points_beta2_epochs250.pdf", width = 15, height = 5)
plot_grid(f1d, f1b, f1c, f1a, nrow = 1)
dev.off()

## Export as separate figures
pdf("../../ICLR_paper/figures/Figure2A_beta2_epochs250_ranked_NA_indices.pdf", 3, 3)
print(f1d)
dev.off()

pdf("../../ICLR_paper/figures/Figure2B_beta2_epochs250_ranked_NA_indices_IS.pdf", 3, 3)
print(f1a)
dev.off()

pdf("../../ICLR_paper/figures/Figure2C_beta2_epochs250_ranked_NA_indices_MWG.pdf", 3, 3)
print(f1b)
dev.off()

pdf("../../ICLR_paper/figures/Figure2D_beta2_epochs250_ranked_NA_indices_PG.pdf", 3, 3)
print(f1c)
dev.off()
...
pdf("../../ICLR_paper/figures/Figure2E_beta2_epochs250_95prcnt_CI_dotplot.pdf", 6, 2.3)
...
pdf("../../ICLR_paper/figures/Figure2F_beta2_epochs250_MAE_dotplot.pdf", 6, 2.3)
...
pdf("../../ICLR_paper/figures/Deviation_from_95CI_beta2_epochs250.pdf", 3, 3)
