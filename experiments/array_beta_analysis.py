...
if __name__=="__main__":
    ...
    for k in range(k_folds):
        ...
        complete_row_ind = np.where(np.isfinite(data_missing_nan).all(axis=1))[0]

        assert set(test_missing_row_ind).union(complete_row_ind) == set(range(len(data_missing_nan)))
    ...
    combined_results = {k: np.array(v).mean() for k,v in combined_results.items()}
    save_results(combined_results, completed_epochs, beta, results_path='beta_analysis3.csv')
    remove_lock()
