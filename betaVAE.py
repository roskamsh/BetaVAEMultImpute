...
from bin.helper_functions import perform_initial_imputation
...
class VariationalAutoencoder(tf.keras.Model):
    def __init__(self, model_settings, initial_imputation_strategy = "zero", l2_regularization=True, 
                 l2_factor = 0.05, pretrained_encoder=None, pretrained_decoder=None, **kwargs):
        """
        ...
        self.initial_imputation_strategy = initial_imputation_strategy 
        if self.dropout:
    ...
    def create_decoder(self):
        ...
        # L2 regularization
        kernel_reg = regularizers.l2(self.l2_factor) if self.l2_regularization else None
        # Decoder layers
        h1 = tf.keras.layers.Dense(decoder_small_n, activation="relu", name='h1', kernel_regularizer=kernel_reg)(latent_inputs)
        n1 = tf.keras.layers.LayerNormalization()(h1)
        if self.dropout:
            n1 = tf.keras.layers.Dropout(self.dropout_rate)(n1)
        
        h2 = tf.keras.layers.Dense(decoder_large_n, activation="relu", name='h2', kernel_regularizer=kernel_reg)(n1)
        n2 = tf.keras.layers.LayerNormalization()(h2)
        if self.dropout:
            n2 = tf.keras.layers.Dropout(self.dropout_rate)(n2)

        x_hat_mean = tf.keras.layers.Dense(self.n_input_nodes, name='x_hat_mean', kernel_regularizer=kernel_reg)(n2)
        x_hat_log_sigma_sq = tf.keras.layers.Dense(self.n_input_nodes, name='x_hat_log_sigma_sq', kernel_regularizer=kernel_reg)(h2)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # Add regularization losses automatically collected by Keras (specified as L2)
            reg_loss = tf.add_n(self.losses) if self.losses else 0
            total_loss = reconstruction_loss + self.beta * kl_loss + reg_loss
    ...
    def calculate_losses(self, true, preds):
        return {
    ...
    def impute_single(self, data_corrupt, data_complete, n_recycles=3, loss='RMSE', scaler=None, return_losses=False):
        ...
        data_miss_val = perform_initial_imputation(data_miss_val, type_imputer = self.initial_imputation_strategy)
        ...
        for i in range(max_iter):
            ...
            if i == 0:
                ...
                z_Distribution = tfp.distributions.Normal(
                    loc=z_mean, scale=tf.sqrt(tf.exp(z_log_sigma_sq))
                    )
                X_hat_distr_s_minus_1 = tfp.distributions.Normal(
                    loc=x_hat_mean_s_minus_1, 
                    scale=np.sqrt(self.beta)*tf.sqrt(tf.exp(x_hat_log_sigma_sq_s_minus_1))
                    )
    ...
    def get_proposal_distribution(self, z_mean, z_log_sigma_sq, proposal, df = 3):
        if proposal == 't':
            scale_diag = tf.sqrt(tf.exp(z_log_sigma_sq))
            z_Distribution = tfp.distributions.MultivariateStudentTLinearOperator(
                df=df,
                loc=z_mean,
    ...
    def sample_datasets_and_compute_ess(self, data_miss_val, na_ind, logweights_byobs, z_sample_l_byobs, max_iter, m):
        sampled_datasets = []
        ess = []
        ...
        for s in range(n_samp):
            ...
            print(f"Sample {s}/{n_samp} ESS: {eff_samp_size}")
    ...
    def impute_sampling_importance_resampling(self, data_miss_val, na_ind, compl_ind, max_iter=10, m=1, proposal = 't', df = 3):
        ...
        z_mean, z_log_sigma_sq, _ = self.encoder.predict(data_miss_val)

        z_Distribution = self.get_proposal_distribution(z_mean=z_mean, z_log_sigma_sq=z_log_sigma_sq, proposal=proposal, df=df)
        logweights, z_sample_l = self.get_samples_and_weights(data_miss_val=data_miss_val, compl_ind=compl_ind, max_iter=max_iter, 
                                                              z_Distribution=z_Distribution, z_prior=z_prior, proposal=proposal)
        ...
        return self.sample_datasets_and_compute_ess(data_miss_val=data_miss_val, na_ind=na_ind,
                                                 logweights_byobs=logweights_byobs, 
                                                 z_sample_l_byobs=z_sample_l_byobs, 
    ...
    def impute_multiple(self, data_corrupt, max_iter=10, m = 1, method = 'pseudo-Gibbs', proposal = 't', df=3):
        ...
        if method == "Metropolis-within-Gibbs":
            return self.impute_metropolis_within_gibbs(data_miss_val=data_miss_val, na_ind=na_ind, max_iter=max_iter) 
        elif method == "sampling-importance-resampling":
            return self.impute_sampling_importance_resampling(data_miss_val=data_miss_val, na_ind=na_ind, compl_ind=compl_ind, 
                                                       max_iter=max_iter, m=m, proposal=proposal, df=df)
        elif method == "pseudo-Gibbs":
            return self.impute_pseudo_gibbs(data_miss_val=data_miss_val, na_ind=na_ind, max_iter=max_iter)
        else:
            raise ValueError(f"Invalid imputation strategy {method} chosen. Please choose a convergence method from either pseudo-Gibbs, Metropolis-within-Gibbs or sampling-importance-resampling")
